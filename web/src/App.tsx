...
const navigation = [
  ...
  { to: "/product-links", label: "Produktabgleich", icon: Link2 },
...
function Shell({
  ...
  const [sidebarExpanded, setSidebarExpanded] = useState(
    () =>
      window.localStorage.getItem("procurement-sidebar-collapsed") !== "true",
  );
  ...
  const horizontalLogo = appAssetPath(
    branding?.assets?.horizontalOnDark ||
      branding?.sidebarLogo ||
      "/logos/procurementcore_white_side.svg",
  );
  const markLogo = appAssetPath(
    branding?.assets?.markOnDark || "/logos/procurementcore_white_icon.svg",
  );
  useEffect(() => {
    window.localStorage.setItem(
      "procurement-sidebar-collapsed",
  ...
  return (
    <div className="shell">
      <aside className={`sidebar ${sidebarExpanded ? "" : "collapsed"}`}>
        <a className="brand" href={dashboardURL}>
          <img
            src={sidebarExpanded ? horizontalLogo : markLogo}
            alt={branding?.productName || "ProcurementCore"}
          />
        </a>
        <button
          ...
          aria-label={
            sidebarExpanded ? "Sidebar zuklappen" : "Sidebar aufklappen"
          }
        ...
        <div className="sidebar-footer">
          <a
            ...
            title={!sidebarExpanded ? "Spend exportieren" : undefined}
          >
            <FileDown size={16} />
          ...
          <div className="user">
            ...
            <div className="user-meta">
              ...
              <div className="user-role">
                {user.isAdmin ? "Einkaufsadministration" : "Anforderer"}
              </div>
            </div>
          </div>
          <button
            ...
            disabled={loggingOut}
            onClick={logout}
            title={!sidebarExpanded ? "Abmelden" : undefined}
          >
            <LogOut size={16} />
      ...
      <main className={`main ${sidebarExpanded ? "" : "sidebar-collapsed"}`}>
        {children}
      </main>
...
export default function App() {
  const [user, setUser] = useState<User | null>(null),
    [branding, setBranding] = useState<Branding | null>(null),
    [loading, setLoading] = useState(true),
    [refreshKey, setRefreshKey] = useState(0),
    [toast, setToast] = useState("");
  useEffect(() => {
    const applyBranding = (b: Branding | null) => {
      if (!b) return;
      setBranding(b);
      const links: [string, string, string | undefined][] = [
        ["link[rel~='icon']", "icon", b.assets?.favicon || b.faviconPath],
        ["link[rel='apple-touch-icon']", "apple-touch-icon", b.assets?.appIcon],
      ];
      links.forEach(([selector, rel, href]) => {
        if (!href) return;
        const assetHref = appAssetPath(href);
        ...
        link.href = assetHref;
        if (rel === "icon")
          link.type = assetHref.toLowerCase().includes(".png")
            ? "image/png"
            : "image/svg+xml";
      });
    };
    Promise.all([
      api<User>("/me"),
      api<Branding>("/branding").catch(() => null),
    ])
      .then(([u, b]) => {
        setUser(u);
        applyBranding(b);
  ...
  if (loading)
    return (
      <div className="empty app-loading">ProcurementCore wird geladen …</div>
    );
  ...
  return (
    <AppContext.Provider value={value}>
      <Shell user={user} branding={branding}>
        <Routes>
