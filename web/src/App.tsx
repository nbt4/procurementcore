...
function Shell({ user, branding, children }:{ user:User; branding:Branding|null; children:React.ReactNode }) {
  ...
  const horizontalLogo = appAssetPath(branding?.assets?.horizontalOnDark || branding?.sidebarLogo || '/logos/procurementcore_white_side.svg')
  const markLogo = appAssetPath(branding?.assets?.markOnDark || '/logos/procurementcore_white_icon.svg')
  ...
  const logout = async () => {
    ...
    try {
      ...
      window.location.assign(new URL('/login', dashboardURL === '/' ? window.location.origin : dashboardURL).toString())
    }
  }
  return <div className="shell">
    <aside className={`sidebar ${sidebarExpanded ? '' : 'collapsed'}`}>
      ...
      <div className="sidebar-footer">
        ...
        <a className="btn ghost sidebar-action" href={dashboardURL} title={!sidebarExpanded?'Cores Dashboard':undefined}><Home size={16}/><span>Cores Dashboard</span></a>
...
export default function App() {
  ...
  useEffect(()=>{
    const applyBranding=(b:Branding|null)=>{ if(!b)return; setBranding(b); const links:[string,string,string|undefined][]=[["link[rel~='icon']",'icon',b.assets?.favicon||b.faviconPath],["link[rel='apple-touch-icon']",'apple-touch-icon',b.assets?.appIcon]]; links.forEach(([selector,rel,href])=>{if(!href)return;const assetHref=appAssetPath(href);let link=document.querySelector<HTMLLinkElement>(selector);if(!link){link=document.createElement('link');link.rel=rel;document.head.appendChild(link)}link.href=assetHref;if(rel==='icon')link.type=assetHref.toLowerCase().includes('.png')?'image/png':'image/svg+xml'}) }
