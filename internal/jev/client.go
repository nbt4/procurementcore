...
// Config contains the OpenRouter-backed Jev connection settings.
type Config struct {
	APIKey   string
	Endpoint string
	Model    string
...
// Client evaluates closed-set decisions with Jev. A nil client means that the
// optional integration is disabled and callers must use their local fallback.
type Client struct {
	apiKey     string
	endpoint   string
	model      string
...
// Decision is a validated Jev Choice response.
...
// Choice describes one independent closed-set question. Multiple choices can
// be evaluated in one request against a small shared state.
type Choice struct {
	Instructions string
...
// FromEnv returns nil unless OPENROUTER_API_KEY is set. JEV_ENABLED=false can
// explicitly disable the integration without removing the secret.
func FromEnv() *Client {
...
// New creates a Jev client. It is exported primarily to allow deterministic
...
// Choose sends one closed-set decision to Jev and validates that the returned
// choice belongs to the supplied criteria. It never logs or returns the key.
...
// ChooseMany evaluates independent choices in one network request. A response
// is accepted only when every answer is present and belongs to its own schema.
